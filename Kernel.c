...
----------------------------- HoareCondition  ---------------------------------
  ...
  behavior HoareCondition
    ...
    --         It unlocks it, and goes to sleep waiting for a signal on
    --         this condition.  When the signal is received, this method
    --         re-awakens and returns.
    --    Signal(mutex)
    --         If there are any threads waiting on this condition, this
    --         method will wake up the oldest and schedule it to run.
    --         Thus, current thread sleeps on the monitor lock and 
    --         the new thread directly gets the lock.
    --    Broadcast(mutex)
    --         This method is like Signal() except that it wakes up all
    --         threads waiting on this condition, not just the next one.
    --    Init()
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	-- Init ()
		...
		-- asigns the initial thread status as unused
	-- Print ()
	...
	threadManagerLock.Lock()
        ...
        if (freeList.IsEmpty())
          ...
          threadInUse.status = JUST_CREATED
  ...
  behavior ProcessManager
	...
	-- GetANewProcess () 
        	 -- This method returns a new ProcessControlBlock; it will wait
       		 -- until one is available.
		 -- If the ProcessControlBlock is available then removes from freeList
		 -- makes the status as Active 
		 -- Assigns a process number
	...
	endFor
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
          ...
          var
        processInUse:ptr to  ProcessControlBlock
	...
	nextPid= nextPid + 1
	processInUse.pid=nextPid
          processInUse.status = ACTIVE
...
-----------------------------  ProcessFinish  --------------------------
 ...
 function InitFirstProcess()
    --This method gets a new thread, initializes it and fork the thread
    -- also calls StartUserProcess

 var th: ptr to Thread

      th=threadManager.GetANewThread()
      th.Init("UserLevelProgramThread")
      th.Fork(StartUserProcess, 0)

 endFunction

 -------------------StartUserProcess-----------------------------------
 function StartUserProcess(procnum: int)

 -- This function starts a user level process

 var
 pNewPCB:ptr to ProcessControlBlock
 pOpenFile:ptr to OpenFile
 initPC:int
 oldStatus:int
...
--------------------------------------------------------------------------------------
 -- step 1
 -- StartUserProcess allocates a new PCB (with GetANewProcess) and connect it with the thread
 -- Init the myThread field in the PCB and the myProcess field in the current thread

 pNewPCB=processManager.GetANewProcess()
 pNewPCB.myThread=currentThread
 currentThread.myProcess=pNewPCB

--------------------------------------------------------------------------------------
 --step 2
 --open the executable file

 pOpenFile=fileManager.Open("TestProgram1")
 if pOpenFile==null
       FatalError("Could Not Open the File ")
 endIf

--------------------------------------------------------------------------------------
 --step 3
 --Creates the logical address space and read the executable into it using OpenFile.LoadExecutable. 
 initPC=pOpenFile.LoadExecutable(&pNewPCB.addrSpace)

--------------------------------------------------------------------------------------
 --step 4
 -- close the file
 fileManager.Close(pOpenFile)

--------------------------------------------------------------------------------------
 --step 5
 --Computes the initial value for the user-level stack
 initUserStackTop=(pNewPCB.addrSpace.numberOfPages* PAGE_SIZE)

--------------------------------------------------------------------------------------
 --step 6
 -- Initialize the stacktop
...
--------------------------------------------------------------------------------------
 ...
 oldStatus=SetInterruptsTo(DISABLED)
 pNewPCB.addrSpace.SetToThisPageTable()
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	-- GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
		-- Allocates number of frames requested and returns its address. If no frames
        	-- are currently available,or it is less than the requested frame wait until the request can be completed
	-- ReturnAllFrames (aPageTable: ptr to AddrSpace)
		-- Adds the frames not in use back to the freeList
	...
	frameManagerLock.Lock()
		...
		if (numWaitingGroups > 1)
                	waitingGroups.Wait(&frameManagerLock)
		endIf
		...
		waitingGroups.Broadcast (&frameManagerLock)
	...
	for i=0 to numFramesToReturn-1
		...
		framesInUse.ClearBit(frameNumber)
		numberFreeFrames=numberFreeFrames+1
	endFor
  ...
  behavior TestHoareSemantic
      ...
      method Init ()
        ...
        threadManagerLock.Init()
         ...
         threadTable[0].Init("A")
         threadTable[1].Init("B")
         threadTable[2].Init("C")
         threadTable[3].Init("D")
         threadTable[4].Init("E")
         threadTable[5].Init("F")
         threadTable[6].Init("G")
         threadTable[7].Init("H")
         threadTable[8].Init("I")
         threadTable[9].Init("J")
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  function Handle_Sys_Exit (returnStatus: int)
       -- Exits a process and prints status
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      FatalError("Syscall shutdown was invoked..")
    endFunction

-----------------------------  Handle_Sys_Yield  ---------------------------------

  function Handle_Sys_Yield ()
       print("Handle_Sys_Yeild invoked\n")
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    ...
    -- LoadExecutable requires an empty addr space, fails if it cannot get the addr space.
    -- After the new addr space has been setup then free up the frames in the previous addr 
    -- space.
	...
	var
       ...
       --------------------------------------------------------------------------------------
       -- Step 1: Transfer the filename string to system space.

       numBytes = currentThread.myProcess.addrSpace.GetStringFromVirtual(
              & strBuffer,          -- kernelAddr
              filename asInteger,   -- virtualAddr
              MAX_STRING_SIZE )
 	...
 	--------------------------------------------------------------------------------------
       ...
       pOpenFile = fileManager.Open (&strBuffer)
       if pOpenFile == null
         return -1
       endIf

       --------------------------------------------------------------------------------------
       -- Step 3: Create the new logical addr space for the exe file
	
       newAddrSpace.Init()
       initPC = pOpenFile.LoadExecutable(&newAddrSpace)
       if initPC < 0
         print ("In Handle_Sys_Exec, could not get newAddrSpace\n")
         return -1
       endIf
	...
	--------------------------------------------------------------------------------------
       -- Step 4: Close the exe file

       fileManager.Close (pOpenFile)
	
	--------------------------------------------------------------------------------------
       -- Step 5: Initilaizes initUserStackTop.

       initUserStackTop = (currentThread.myProcess.addrSpace.numberOfPages * PAGE_SIZE)
	
	--------------------------------------------------------------------------------------
       -- Step 6: Reset system stack top to full size

       initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE-1]
       
	--------------------------------------------------------------------------------------
	--step 7: Make this thread a user thread so it can execute the exe file.
	oldStatus = SetInterruptsTo (DISABLED)
        currentThread.isUserThread = true 
       BecomeUserThread(initUserStackTop, initPC, initSystemStackTop asInteger)
       return 3000
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
       var
               ...
               numBytes= currentThread.myProcess.addrSpace.GetStringFromVirtual(
                       ...
                       MAX_STRING_SIZE)
               if numBytes<0
                       FatalError("Error using GetStringFromVirtual")
               endIf

               print("Handle_Sys_Create invoked..\n")

               printHexVar ("Virtual address of filename", filename asInteger)
               print ("filename = ")
                       print (& strBuffer)
                       nl ()
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
       ...
       numBytes = currentThread.myProcess.addrSpace.GetStringFromVirtual(
              & strBuffer, filename asInteger, MAX_STRING_SIZE )
       if numBytes < 0
         FatalError("ERROR attempting to GetStringFromVirtual")
       endIf
